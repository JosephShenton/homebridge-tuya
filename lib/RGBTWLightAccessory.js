...
class RGBTWLightAccessory extends BaseAccessory {
  ...
  _registerCharacteristics(dps) {
    ...
    console.log(
      `[Tuya] > Device Context: ${JSON.stringify(this.device.context)}`
    );
    console.log(`[Tuya] > Device State: ${JSON.stringify(this.device.state)}`);
    ...
    if (this.device.context.cmdWhite) {
      if (/^w[a-z]+$/i.test(this.device.context.cmdWhite))
        ...
        throw new Error(
          `The cmdWhite doesn't appear to be valid: ${this.device.context.cmdWhite}`
        );
    }
    ...
    if (this.device.context.cmdColor) {
      if (/^c[a-z]+$/i.test(this.device.context.cmdColor))
        ...
        throw new Error(
          `The cmdColor doesn't appear to be valid: ${this.device.context.cmdColor}`
        );
      ...
      if (/^c[a-z]+$/i.test(this.device.context.cmdColour))
        ...
        throw new Error(
          `The cmdColour doesn't appear to be valid: ${this.device.context.cmdColour}`
        );
    }
    ...
    const characteristicHue = service
      ...
      .updateValue(
        dps[this.dpMode] === this.cmdWhite
          ? 0
          : this.convertColorFromTuyaToHomeKit(dps[this.dpColor]).h
      )
    ...
    const characteristicSaturation = service
      ...
      .updateValue(
        dps[this.dpMode] === this.cmdWhite
          ? 0
          : this.convertColorFromTuyaToHomeKit(dps[this.dpColor]).s
      )
    ...
    this.device.on("change", (changes, state) => {
      if (
        changes.hasOwnProperty(this.dpPower) &&
        characteristicOn.value !== changes[this.dpPower]
      )
      ...
      switch (state[this.dpMode]) {
        case this.cmdWhite:
          if (
            ...
            this.convertBrightnessFromHomeKitToTuya(
              characteristicBrightness.value
            ) !== changes[this.dpBrightness]
          )
            characteristicBrightness.updateValue(
              this.convertBrightnessFromTuyaToHomeKit(
                changes[this.dpBrightness]
              )
            );
          ...
          if (
            ...
            this.convertColorTemperatureFromHomeKitToTuya(
              characteristicColorTemperature.value
            ...
            const newColor =
              this.convertHomeKitColorTemperatureToHomeKitColor(
                newColorTemperature
              );
  ...
  getBrightness(callback) {
    if (this.device.state[this.dpMode] === this.cmdWhite)
      return callback(
        null,
        this.convertBrightnessFromTuyaToHomeKit(
          this.device.state[this.dpBrightness]
        )
      );
    callback(
      null,
      this.convertColorFromTuyaToHomeKit(this.device.state[this.dpColor]).b
    );
  }
  ...
  setBrightness(value, callback) {
    ...
    this.setState(
      this.dpColor,
      this.convertColorFromHomeKitToTuya({ b: value }),
      callback
    );
  }
  ...
  _setHueSaturation(prop, callback) {
    ...
    const isSham =
      this._pendingHueSaturation.props.h === 0 &&
      this._pendingHueSaturation.props.s === 0;
    ...
    this.setMultiState(
      { [this.dpMode]: this.cmdColor, [this.dpColor]: newValue },
      callEachBack
    );
  }
