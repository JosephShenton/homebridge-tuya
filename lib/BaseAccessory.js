class BaseAccessory {
  constructor(...props) {
    ...
    this.device.once("change", () => {
      this.log(
        `Ready to handle ${this.device.context.name} (${
          this.device.context.type
        }:${this.device.context.version}) with signature ${JSON.stringify(
          this.device.state
        )}`
      );
  ...
  _getCustomDP(numeral) {
    return typeof numeral !== "undefined" &&
      isFinite(numeral) &&
      parseInt(numeral) > 0
      ? String(numeral)
      : false;
  ...
  _detectColorFunction(value) {
    this.colorFunction =
      ...
      { HSB: "HSB", HEXHSB: "HEXHSB" }[
        this.device.context.colorFunction.toUpperCase()
      ];
    if (!this.colorFunction && value) {
      this.colorFunction =
        { 12: "HSB", 14: "HEXHSB" }[value.length] || "Unknown";
      if (this.colorFunction)
        console.log(
          `[Tuya] Color format for ${this.device.context.name} (${this.device.context.version}) identified as ${this.colorFunction} (length: ${value.length}).`
        );
    }
    if (!this.colorFunction) {
      this.colorFunction = "Unknown";
      console.log(
        `[Tuya] Color format for ${this.device.context.name} (${this.device.context.version}) is undetectable.`
      );
  ...
  convertColorTemperatureFromHomeKitToTuya(value) {
    ...
    const convertedValue = Math.round(
      ((scale * min) / (max - min)) * (max / adjustedValue - 1)
    );
  ...
  convertColorTemperatureFromTuyaToHomeKit(value) {
    ...
    const convertedValue = Math.round(
      ((unadjustedValue - 153) * (600 - 71)) / (max - min) + 71
    );
  ...
  convertColorFromHomeKitToTuya_HEXHSB(value, dpValue) {
    const cached = this.convertColorFromTuya_HEXHSB_ToHomeKit(
      dpValue || this.device.state[this.dpColor]
    );
  ...
  convertColorFromHomeKitToTuya_HSB(value, dpValue) {
    const cached = this.convertColorFromTuya_HSB_ToHomeKit(
      dpValue || this.device.state[this.dpColor]
    );
    let { h, s, b } = { ...cached, ...value };
    return (
  ...
  convertColorFromTuya_HEXHSB_ToHomeKit(value) {
    const [, h, s, b] = (value || "0000000000ffff").match(
  ...
  convertColorFromTuya_HSB_ToHomeKit(value) {
    const [, h, s, b] = (value || "000003e803e8").match(
  ...
  convertHomeKitColorTemperatureToHomeKitColor(value) {
    ...
    const rgb = [
      dKelvin > 66
        ? 351.97690566805693 +
          0.114206453784165 * (dKelvin - 55) -
          40.25366309332127 * Math.log(dKelvin - 55)
        : 255,
      dKelvin > 66
        ? 325.4494125711974 +
          0.07943456536662342 * (dKelvin - 50) -
          28.0852963507957 * Math.log(dKelvin - 55)
        : 104.49216199393888 * Math.log(dKelvin - 2) -
          0.44596950469579133 * (dKelvin - 2) -
          155.25485562709179,
      dKelvin > 66
        ? 255
        : 115.67994401066147 * Math.log(dKelvin - 10) +
          0.8274096064007395 * (dKelvin - 10) -
          254.76935184120902,
