...
class AirConditionerAccessory extends BaseAccessory {
  ...
  constructor(...props) {
    ...
    if (this.device.context.cmdCool) {
      if (/^c[a-z]+$/i.test(this.device.context.cmdCool))
        this.cmdCool = ("" + this.device.context.cmdCool).trim();
      else
        throw new Error(
          "The cmdCool doesn't appear to be valid: " +
            this.device.context.cmdCool
        );
    }

    this.cmdHeat = "HEAT";
    if (this.device.context.cmdHeat) {
      if (/^h[a-z]+$/i.test(this.device.context.cmdHeat))
        this.cmdHeat = ("" + this.device.context.cmdHeat).trim();
      else
        throw new Error(
          "The cmdHeat doesn't appear to be valid: " +
            this.device.context.cmdHeat
        );
    }

    this.cmdAuto = "AUTO";
    if (this.device.context.cmdAuto) {
      if (/^a[a-z]+$/i.test(this.device.context.cmdAuto))
        this.cmdAuto = ("" + this.device.context.cmdAuto).trim();
      else
        throw new Error(
          "The cmdAuto doesn't appear to be valid: " +
            this.device.context.cmdAuto
        );
    }
  ...
  _registerCharacteristics(dps) {
    ...
    this.dpThreshold =
      this._getCustomDP(this.device.context.dpThreshold) || "2";
    ...
    this.dpRotationSpeed =
      this._getCustomDP(this.device.context.dpRotationSpeed) || "5";
    this.dpChildLock =
      this._getCustomDP(this.device.context.dpChildLock) || "6";
    this.dpTempUnits =
      this._getCustomDP(this.device.context.dpTempUnits) || "19";
    this.dpSwingMode =
      this._getCustomDP(this.device.context.dpSwingMode) || "104";
    ...
    if (!this.device.context.noCool)
      _validTargetHeaterCoolerStateValues.unshift(
    ...
    this.characteristicCoolingThresholdTemperature =
    ...
    this.device.on("change", (changes, state) => {
      if (changes.hasOwnProperty(this.dpActive)) {
        ...
        if (characteristicActive.value !== newActive) {
          ...
          if (!changes.hasOwnProperty(this.dpMode)) {
            characteristicCurrentHeaterCoolerState.updateValue(
              this._getCurrentHeaterCoolerState(state)
            );
          }

          if (!changes.hasOwnProperty(this.dpRotationSpeed)) {
            characteristicRotationSpeed.updateValue(
              this._getRotationSpeed(state)
            );
          }
        }
      }
      ...
      if (
        characteristicLockPhysicalControls &&
        changes.hasOwnProperty(this.dpChildLock)
      ) {
        const newLockPhysicalControls = this._getLockPhysicalControls(
          changes[this.dpChildLock]
        );
        if (
          characteristicLockPhysicalControls.value !== newLockPhysicalControls
        ) {
          characteristicLockPhysicalControls.updateValue(
            newLockPhysicalControls
          );
        }
      }
      ...
      if (changes.hasOwnProperty(this.dpThreshold)) {
        if (
          ...
          characteristicCoolingThresholdTemperature &&
          characteristicCoolingThresholdTemperature.value !==
            changes[this.dpThreshold]
        )
          characteristicCoolingThresholdTemperature.updateValue(
            changes[this.dpThreshold]
          );
        if (
      ...
      if (changes.hasOwnProperty(this.dpMode)) {
        const newTargetHeaterCoolerState = this._getTargetHeaterCoolerState(
          changes[this.dpMode]
        );
        const newCurrentHeaterCoolerState =
          this._getCurrentHeaterCoolerState(state);
        if (
          characteristicTargetHeaterCoolerState.value !==
          newTargetHeaterCoolerState
        )
          characteristicTargetHeaterCoolerState.updateValue(
            newTargetHeaterCoolerState
          );
        if (
          characteristicCurrentHeaterCoolerState.value !==
      ...
      if (changes.hasOwnProperty(this.dpSwingMode)) {
        ...
        if (characteristicSwingMode.value !== newSwingMode)
          characteristicSwingMode.updateValue(newSwingMode);
      }

      if (changes.hasOwnProperty(this.dpTempUnits)) {
        const newTemperatureDisplayUnits = this._getTemperatureDisplayUnits(
          changes[this.dpTempUnits]
        );
        if (
          characteristicTemperatureDisplayUnits.value !==
          newTemperatureDisplayUnits
        )
          characteristicTemperatureDisplayUnits.updateValue(
            newTemperatureDisplayUnits
          );
      }
  ...
  _getRotationSpeed(dps) {
    ...
    if (this._hkRotationSpeed) {
      const currntRotationSpeed = this.convertRotationSpeedFromHomeKitToTuya(
    ...
    return (this._hkRotationSpeed = this.convertRotationSpeedFromTuyaToHomeKit(
  ...
  setRotationSpeed(value, callback) {
    ...
    if (value === 0) {
      ...
      this.setMultiState(
        {
          dpActive: true,
          dpRotationSpeed: this.convertRotationSpeedFromHomeKitToTuya(value),
        },
        callback
      );
    }
  ...
  convertRotationSpeedFromHomeKitToTuya(value) {
    return this.device.context.fanSpeedSteps
